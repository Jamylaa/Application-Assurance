...
public class ValidationService {
    ...
    private static final Pattern NOM_PATTERN = Pattern.compile("^[\\p{L}0-9\\s\\-_'&()]{2,100}$");
    private static final Pattern DESCRIPTION_PATTERN = Pattern.compile("^[\\p{L}0-9\\s\\-_',.]{10,500}$");
    ...
    public ValidationResult validatePack(tn.vermeg.gestionproduit.entities.Pack pack) {
        ...
        if (pack.getAgeMinimum() != null && pack.getAgeMaximum() != null) {
            // Rejeter les âges 0-0 qui indiquent un échec d'extraction
            if (pack.getAgeMinimum() == 0 && pack.getAgeMaximum() == 0) {
                result.addError("Les âges minimum et maximum ne sont pas spécifiés. Veuillez indiquer la tranche d'âge (ex: 60-85 ans)");
            }
        ...
        } else if (pack.getAgeMinimum() == 0 || pack.getAgeMaximum() == 0) {
            // Au moins un des âges est 0, ce qui indique un problème
            result.addError("La tranche d'âge est incomplète. Veuillez spécifier à la fois l'âge minimum et maximum");
        }
        ...
        if (pack.getCouvertureGeographique() == null) {
            result.addError("La couverture géographique est obligatoire. Veuillez spécifier : NATIONAL, UE, MAGHREB, INTERNATIONAL ou REGIONAL");
        }
        // Validation des durées de contrat
        ...
        if (pack.getNiveauCouverture() == null) {
            result.addError("Le niveau de couverture est obligatoire. Veuillez spécifier : BASIC, PREMIUM ou GOLD");
        }
        // Validation du statut
