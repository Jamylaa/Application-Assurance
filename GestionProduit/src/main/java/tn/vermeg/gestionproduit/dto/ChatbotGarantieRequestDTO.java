...
public class ChatbotGarantieRequestDTO {
    ...
    public void generateBusinessHash() {
        if (this.nomGarantie != null && this.domaine != null) {
            this.businessHash = (this.nomGarantie + "_" + this.domaine.name()).toUpperCase().replaceAll("[^A-Z0-9_]", "_");
        }
    }

    @Override
    public String toString() {
        return String.format("ChatbotGarantieRequestDTO{nom='%s', domaine='%s', taux=%.2f, warnings=%d}", 
                           nomGarantie, domaine != null ? domaine.name() : null, tauxRemboursement, warnings.size());
    }
}
